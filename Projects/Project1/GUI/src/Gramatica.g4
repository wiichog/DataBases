...
literal : int_literal | float_literal | char_literal | fecha_literal  ;

int_literal: (NUMX|DIGIT|DIGIT_four|DIGIT_two);
float_literal: (NUMX|DIGIT|DIGIT_four|DIGIT_two) '.' (NUMX|DIGIT|DIGIT_four|DIGIT_two);
char_literal: APOS (CHARX|IDX) APOS;
fecha_literal:  APOS DIGIT_four '-' DIGIT_two '-' DIGIT_two APOS ;




programa: char_literal|database|(database)*;
...
tipo: INT|FLOAT|DATE|CHAR '(' (NUMX|DIGIT|DIGIT_two|DIGIT_four) ')' ;

createTable: CREATE TABLE IDX '(' columna (',' columna)* (CONSTRAINT constraint (',' CONSTRAINT constraint)*)? ')';
...
updateSet: UPDATE IDX SET updateCol (',' updateCol)* (WHERE exp)? ;
updateCol: IDX '=' literal;
